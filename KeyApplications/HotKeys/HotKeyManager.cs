...
namespace KeyBindingServiceMeow.KeyApplications.HotKeys
{
    ...
    {
        public static List<HotKeyManager> hotKeyManagers = new List<HotKeyManager>();
        ...
        public static void Create(Player player)
        {
            if (hotKeyManagers.Any(x => x.player == player))
                throw new Exception("A hot key manager is already created for " + player.Nickname);

            new HotKeyManager(player);

            Log.Debug("HotKeyManager Created for " + player.Nickname);
        }
        ...
        public static void Destruct(Player player)
        {
            ...
            Log.Debug("HotKeyManager Destructed for " + player.Nickname);
        }
        ...
        {
            SettingManager.instance.ChangeSettings(player.Nickname, setting);
        ...
        {
            hotKey = hotKeys.FirstOrDefault(x => x.id == id);
            return hotKey != null;
        }

        //Internal Setting Update Methods
        public void UpdateAllKeySetting()
        {
            foreach(var hotKey in hotKeys)
            {
                UpdateKeySetting(hotKey);
        ...
        public void UpdateKeySetting(HotKey hotKey)
        {
            UpdateKeySetting(hotKey.id);
        }

        public void UpdateKeySetting(string id)
        {
            var hotKey = hotKeys.Find(x => x.id == id);

            var setting = SettingManager.instance.GetSettings(player.Nickname)
